...
test("an expired access token is silently refreshed on reload, with no visible failure", async ({ page }) => {
  ...
  const accessBefore = await page.evaluate(() => localStorage.getItem("vaultvista_access_token"));
  ...
  await page.evaluate(() => localStorage.setItem("vaultvista_access_token", "expired.garbage.token"));
  await page.reload();
  ...
  const accessAfter = await page.evaluate(() => localStorage.getItem("vaultvista_access_token"));
...
test("a completely invalid refresh token forces the user back to the login screen", async ({ page }) => {
  ...
  await page.evaluate(() => {
    localStorage.setItem("vaultvista_access_token", "expired.garbage.token");
    localStorage.setItem("vaultvista_refresh_token", "never-issued-refresh-token");
  });
  await page.reload();
